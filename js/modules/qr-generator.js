import { limpiarId, mostrarMensaje, sanitizarNombre, debounce, generarQR } from '../shared/utils.js';
...
export async function initQR() {
    await cargarExcel();
    setupEventListenersQR();
}

async function cargarExcel() {
    mostrarMensaje(elements.messageId, 'Cargando archivo Excel para QR...', 'info', 0);
    try {
        ...
        filteredData = [];
        poblarSelect();
        mostrarMensaje(elements.messageId, `✅ ${excelData.length} activos cargados para QR`, 'success');
    } catch (e) {
        console.error('Error QR:', e);
        mostrarMensaje(elements.messageId, `⚠️ Error al cargar QR: ${e.message}`, 'error');
    }
}
...
function setupEventListenersQR() {
    ...
    elements.activoSelect.addEventListener('change', function() {
...
async function handleGenerate() {
    if (!currentItem) {
        mostrarMensaje(elements.messageId, '❌ Primero selecciona un activo', 'error');
        return;
    }
    ...
    try {
        ...
        mostrarMensaje(elements.messageId, '✅ Tarjeta generada correctamente', 'success');
    } catch (error) {
        console.error('Error:', error);
        mostrarMensaje(elements.messageId, '❌ Error al generar la tarjeta', 'error');
    } finally {
...
async function handleDownload() {
    ...
    try {
        ...
        if (!img) {
            const { id, codigo, desc } = currentItem;
            img = await generarImagenTarjeta(id, codigo, desc);
        }
        const link = document.createElement('a');
        const nombre = sanitizarNombre(currentItem.codigo);
        link.download = `tarjeta-${nombre}.png`;
        link.href = img;
        link.click();
        mostrarMensaje(elements.messageId, '📥 Tarjeta descargada', 'success');
    } catch (error) {
        mostrarMensaje(elements.messageId, '❌ Error al descargar', 'error');
    }
}
    ...
    try {
        ...
        if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
                title: 'Tarjeta QR',
                text: `Tarjeta para ${currentItem.codigo}`,
                files: [file]
            });
            mostrarMensaje(elements.messageId, '📤 Compartido correctamente', 'success');
        } else {
            mostrarMensaje(elements.messageId, '📱 Compartir no soportado, se descargará', 'info');
            handleDownload();
        }
    } catch (error) {
